...
**Barge-in** — Nova is otherwise half-duplex (the mic gate is closed during playback so it never hears its own TTS). `stt_engine.start_barge_listen()` routes those otherwise-discarded frames to a SEPARATE detector instance while Nova speaks, so saying the wake word interrupts it (`nova._on_barge_in` stops TTS; the buffered audio primes the next command so the user's words aren't clipped). **Self-trigger guarantee:** barge-in is never armed for a response whose text contains the wake word (`nova._says_wake_word`), and in the streaming path a sentence is seen before it is synthesized — so Nova saying "Nova" can never interrupt itself. A stricter threshold (0.7 vs 0.5) is the acoustic margin on top. Tunable/disable via `config.wake_word.barge_in`.
