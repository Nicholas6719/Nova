...
**Barge-in** — Nova is otherwise half-duplex (the mic gate is closed during playback so it never hears its own TTS). `stt_engine.start_barge_listen()` routes those otherwise-discarded frames to a SEPARATE detector instance while Nova speaks, so saying the wake word interrupts it (`nova._on_barge_in` stops TTS; the buffered audio primes the next command so the user's words aren't clipped). **Self-trigger guarantee:** barge-in is never armed for a response whose text contains the wake word (`nova._says_wake_word`), and in the streaming path a sentence is seen before it is synthesized — so Nova saying "Nova" can never interrupt itself. **Barge-in is DISABLED by default — it does not work over speakers.** Measured: the wake model's score collapses once Nova's own voice reaches the mic (0.96 clean → 0.50 at 5% echo → 0.003 at 10% → 0.00 above). A laptop's speakers sit inches from the mic, so the echo far exceeds that. This is not a tuning issue (scores are ~0, not borderline) — true barge-in requires acoustic echo cancellation (webrtc-audio-processing/speexdsp, or macOS VoiceProcessingIO). It DOES work on headphones; enable it via `config.wake_word.barge_in` there.
